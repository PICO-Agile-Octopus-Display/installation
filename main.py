...
class Display:
    ...
    def __init__(self, ctrl, display):
        ...
        self.status = DisplayItem(0, self.height-self.FONT_HEIGHT, self.display, self.DRAW_COL,
                                  self.ERASE_COL, self.FONT, self.FONT_SIZE, round(self.width/2), 32, DisplayItem.LEFT)
        self.clock = DisplayItem(round(self.width/2), self.height-self.FONT_HEIGHT, self.display, self.DRAW_COL,
                                 self.ERASE_COL, self.FONT, self.FONT_SIZE, round(self.width/2), self.FONT_HEIGHT, DisplayItem.RIGHT)
        w = 25  # width of min and max
        self.min = DisplayItem(self.width-w, self.MIN_Y, self.display, self.DRAW_COL,
                               self.ERASE_COL, self.FONT, self.FONT_SIZE, w, self.FONT_HEIGHT, DisplayItem.RIGHT)
        self.max = DisplayItem(self.width-w, self.MAX_Y, self.display, self.DRAW_COL,
                               self.ERASE_COL, self.FONT, self.FONT_SIZE, w, self.FONT_HEIGHT, DisplayItem.RIGHT)
        self.tariff = DisplayItem(0, 0, self.display, self.DRAW_COL, self.ERASE_COL, self.TARIFF_FONT,
                                  self.TARIFF_FONT_SIZE, self.width, self.TARIFF_FONT_HEIGHT, DisplayItem.CENTRE)
...
class InkDisplay(Display):
    ...
    def do_bars(self, bars, bars_length, start):
        ...
        self.display.set_pen(self.ERASE_COL)
        self.display.rectangle(self.BAR_X, self.BAR_Y,
                               self.BAR_AREA_WIDTH, self.BAR_AREA_HEIGHT)
        ...
        for i in range(bars_length):
            ...
            self.display.rectangle(round(bar_x), round(
                bar_y), round(bar_width), round(bar_height))
        self.display_dirty = True
...
class Agile:
    ...
    def do_get_daily_rates(self, time_mgr):
        ...
        for result in results:
            ...
            start_secs = time.mktime(
                (t[0], t[1], t[2], t[3], t[4], 0, 0, 0, -1))
            if time_mgr.summer:
                start_secs = start_secs+3600
...
class Connection:
    ...
    def do_connect(self, ssid, pwd):
        ...
        try:
            ...
            self.ctrl.display.draw()
            print("Local time before synchronization:%s" %
                  str(time.localtime()))
            # make sure to have internet connection
            ntptime.settime()
            print("Local time after synchronization:%s" %
                  str(time.localtime()))
        except:
...
class Time_Manager:
    ...
    def update_time(self):
        ...
        bst_start_secs = utime.mktime(
            (t[0], 3, 31 - days_to_subtract, 1, 0, 0, 0, 0, -1))
        #
        ...
        bst_end_secs = utime.mktime(
            (t[0], 10, 31 - days_to_subtract, 1, 0, 0, 0, 0, -1))
        #
...
class Manager:
    ...
    def __init__(self):
        ...
        print("https://github.com/PICO-Agile-Octopus-Display/installation")
        self.config = Config()
        self.config.load()
    ...
    def do_update(self):
        ...
        self.rate = self.agile.do_get_rate(self.time_mgr)
        if self.rate == None:
            self.display.do_tariff("")
            self.display.do_status("No rate data")
        else:
            self.display.do_tariff("%.2f" % (self.rate))
        self.display.do_bars(self.agile.half_hour_prices,
                             48, self.agile.offset)
